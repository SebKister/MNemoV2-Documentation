...
- **Compass calibration shows coverage instead of a reading count**: the calibration screen now shows a **COVERAGE** figure and bar, red until the readings pin the compass model down, yellow from 50% when a save becomes possible, green from 90%, the target. Once a save is possible the figure is the best coverage reached so far, and that is the calibration SAVE CAL. stores, so it cannot drop while you reach for the button. Turning the MNemo flat on the spot filled the old progress bar just as well as a proper tumble, yet gave a calibration that could be wrong by tens of degrees while reporting a perfect fit error; that is exactly what the coverage figure detects. See [Calibrating the compass](Calibrating-the-compass.md).
- **SAVE is refused on low coverage**: pressing SAVE CAL. too early shows **LOW COVERAGE** and keeps the calibration running. Keep nodding and rolling the device until the bar changes colour.
- **The result stays on screen after a save**, with a grade of EXCELLENT, GOOD or FAIR next to the coverage, fit error, field strength and hard-iron offset that were stored, until you press SELECT.
...
- Fixed marker counter initialisation bug
- Added button threshold auto-calibration (4 consecutive clicks on either side)
- Settings are now saved when modified from the web interface
